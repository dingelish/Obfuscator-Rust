...
This repository acts as a specialized fork of [Obfuscator-LLVM](https://github.com/obfuscator-llvm/obfuscator) and [Obfuscator-LLVM-16.0](https://github.com/joaovarelas/Obfuscator-LLVM-16.0), engineered to support **LLVM 22.1** and **Rust 1.98.1**.
...
*   **Changed:** Ported the toolchain to **Rust 1.98.1** / **LLVM 22.1** (from Rust 1.92.0 / LLVM 21.1). Rust 1.98.1 pins `rust-lang/llvm-project` branch `rustc/22.1-2026-05-19`. The pass-pipeline registration in `PassBuilderPipelines.cpp` was re-based onto LLVM 22 (which added MemProf handling in the default pipelines), and one LLVM 22 API change was addressed: `CallInst::Create` no longer accepts `std::nullopt` for empty argument lists (`ArrayRef` dropped its `std::nullopt_t` conversion), now `{}`.
